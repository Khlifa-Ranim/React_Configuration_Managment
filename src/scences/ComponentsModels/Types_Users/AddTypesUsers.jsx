...
const AddRole = () => {    
  ...
  const dispatch = useDispatch();
   ...
   const CreateTypeUserHandle = (e) => {
    e.preventDefault();
    setFromErrors(validate(name, description));
    setIsSubmit(true);
  
    if (Object.keys(formErrors).length === 0 && name.trim() !== '' && description.trim() !== '') {
      dispatch(CreateTypesUsres({ name, description }));
      notify(); // display toast notification
      setTimeout(() => navigate('/FeatchTypesUsers'), 3000); // redirect after 3 seconds
    }
  };
  ...
  return (<>
    ...
    <Sidebar/> 
     ...
     <Topbar/>
      ...
      <form className="MainContainer2" onSubmit={CreateTypeUserHandle}>
      <ToastContainer />
      <h2 className="WelcomeText">Add  a New Type For A Users</h2>
